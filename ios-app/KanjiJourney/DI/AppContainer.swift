...
final class AppContainer: ObservableObject {
    ...
    let database: KanjiJourneyDatabase
    ...
    init() {
        ...
        // registerBackgroundTask() is called from KanjiJourneyApp.init or .task.
    ...
    private static let dbName = "kanjijourney.db"
    private static let dbVersionKey = "kanjijourney_swift_db_version"
    ...
    private static func stageBundleDatabase() {
        ...
        guard let docsURL = fm.urls(for: .documentDirectory, in: .userDomainMask).first else {
            NSLog("KanjiJourney [Swift]: Cannot resolve Documents directory")
            return
        }
        ...
        if fm.fileExists(atPath: dest.path) && installedVersion >= dbVersion {
            if let attrs = try? fm.attributesOfItem(atPath: dest.path),
               let size = attrs[.size] as? UInt64, size > 1_000_000 {
                NSLog("KanjiJourney [Swift]: DB already staged (v%d, %llu bytes)", installedVersion, size)
                return
            }
            NSLog("KanjiJourney [Swift]: DB file too small (%@), forcing re-copy", dest.path)
        }

        guard let bundleURL = Bundle.main.url(forResource: "kanjijourney", withExtension: "db") else {
            NSLog("KanjiJourney [Swift]: DB NOT FOUND in app bundle!")
            if let resourcePath = Bundle.main.resourcePath {
                let contents = (try? fm.contentsOfDirectory(atPath: resourcePath)) ?? []
                NSLog("KanjiJourney [Swift]: Bundle contains %d items: %@",
                      contents.count,
                      contents.filter { $0.hasSuffix(".db") || $0 == "kanjijourney.db" }.joined(separator: ", "))
            }
            return
        }

        NSLog("KanjiJourney [Swift]: Found bundle DB at %@", bundleURL.path)

        if fm.fileExists(atPath: dest.path) {
            do {
                try fm.removeItem(at: dest)
            } catch {
                NSLog("KanjiJourney [Swift]: Failed to remove stale DB: %@", error.localizedDescription)
            }
        }
        ...
        do {
            ...
            NSLog("KanjiJourney [Swift]: DB copied successfully (%llu bytes)", size)
        } catch {
            NSLog("KanjiJourney [Swift]: Failed to copy DB: %@", error.localizedDescription)
        }
    }
}
