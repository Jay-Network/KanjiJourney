...
final class SyncService: ObservableObject {
    ...
    static let backgroundTaskIdentifier = "com.jworks.kanjijourney.sync"
        ...
        Task {
            ...
            do {
                ...
                if let success = result as? SyncResult.Success {
                    NSLog("KanjiJourney [Sync]: App open sync complete — pushed=\(success.pushed), pulled=\(success.pulled)")
                }
            } catch {
                NSLog("KanjiJourney [Sync]: App open sync failed: \(error.localizedDescription)")
            }
        }
    }
    ...
    func registerBackgroundTask() -> Bool {
        ...
        NSLog("KanjiJourney [Sync]: BGTask register=%@", ok ? "true" : "false")
        return ok
    }
    ...
    func scheduleBackgroundSync() {
        ...
        do {
            ...
            NSLog("KanjiJourney [Sync]: Background sync scheduled")
        } catch {
            NSLog("KanjiJourney [Sync]: Failed to schedule background sync: \(error.localizedDescription)")
        }
    }
    ...
    private func handleBackgroundSync(task: BGAppRefreshTask) {
        ...
        let syncTask = Task {
            ...
            do {
                ...
                NSLog("KanjiJourney [Sync]: Background sync \(success ? "succeeded" : "failed")")
            } catch {
                task.setTaskCompleted(success: false)
                NSLog("KanjiJourney [Sync]: Background sync error: \(error.localizedDescription)")
            }
        }
        ...
        Task {
            ...
            do {
                ...
                if let deviceId = deviceId {
                    NSLog("KanjiJourney [Sync]: Device registered: %@", deviceId)
                }
            } catch {
                NSLog("KanjiJourney [Sync]: Device registration failed: \(error.localizedDescription)")
            }
        }
    }
    ...
    private func getUserId() async -> String? {
        do {
            ...
            NSLog("KanjiJourney [Sync]: getUserId() failed: %@", error.localizedDescription)
            return nil
        }
    }
}
