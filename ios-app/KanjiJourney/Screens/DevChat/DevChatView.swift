...
struct DevChatView: View {
                    ...
                    Button {
                        ...
                        if viewModel.isSending {
                            ...
                            Image(systemName: "paperplane.fill")
                                .foregroundColor(messageText.trimmingCharacters(in: .whitespaces).isEmpty ? KanjiJourneyTheme.onSurfaceVariant : KanjiJourneyTheme.primary)
                        }
                    }
    ...
    private func messageBubble(_ message: DevChatMessage) -> some View {
        ...
        let bubbleColor = isFromUser ? KanjiJourneyTheme.primaryContainer : KanjiJourneyTheme.secondaryContainer
        let textColor = isFromUser ? KanjiJourneyTheme.onPrimaryContainer : KanjiJourneyTheme.onSecondaryContainer
        ...
        return VStack(alignment: isFromUser ? .trailing : .leading, spacing: 2) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                if let cat = message.category {
                    Text(cat.label)
                        .font(KanjiJourneyTheme.labelSmall).fontWeight(.bold)
