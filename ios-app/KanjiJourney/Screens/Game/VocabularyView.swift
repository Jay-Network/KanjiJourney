...
struct VocabularyView: View {
    ...
    private var loadingContent: some View {
        VStack(spacing: KanjiJourneyTheme.spacingM) {
            Text("Preparing vocabulary...").font(KanjiJourneyTheme.titleMedium)
            ProgressView()
        }
    }
    ...
    private func questionView(_ question: Question) -> some View {
        ScrollView {
            VStack(spacing: 0) {
                // Progress
                progressRow.padding(.horizontal, KanjiJourneyTheme.spacingM).padding(.top, KanjiJourneyTheme.spacingS)
        ...
        VStack(spacing: 4) {
            ...
            case "meaning":
                ...
                if let reading = question.vocabReading {
                    Text(reading).font(KanjiJourneyTheme.titleMedium).foregroundColor(KanjiJourneyTheme.onSurfaceVariant)
                }
            case "reading":
                Text(question.kanjiLiteral).font(.system(size: 48)).multilineTextAlignment(.center)
    ...
    private func decompositionCard(_ breakdown: [String]) -> some View {
        ...
        return VStack(spacing: 8) {
            ...
            HStack(spacing: 0) {
                ForEach(Array(components.enumerated()), id: \.offset) { index, component in
                    VStack(spacing: 2) {
                        KanjiText(text: component.kanji, font: .system(size: 32, weight: .bold, design: .serif), color: KanjiJourneyTheme.primary)
        ...
        .padding(12)
        .background(KanjiJourneyTheme.surfaceVariant.opacity(0.5))
        .cornerRadius(KanjiJourneyTheme.radiusM)
    }
    ...
    private func exampleSentenceCard(ja: String, en: String?) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("Example")
                .font(KanjiJourneyTheme.labelMedium)
                .foregroundColor(KanjiJourneyTheme.onSurfaceVariant)
            Text(ja).font(KanjiJourneyTheme.bodyMedium)
            if let en = en {
                Text(en).font(KanjiJourneyTheme.bodySmall).foregroundColor(KanjiJourneyTheme.onSurfaceVariant)
            }
        }
        ...
        VStack(spacing: KanjiJourneyTheme.spacingL) {
            ...
            Button {
                ...
                Text("Done").font(KanjiJourneyTheme.labelLarge).fontWeight(.bold)
                    ...
                    .background(KanjiJourneyTheme.primary).cornerRadius(KanjiJourneyTheme.radiusM)
            }
            .padding(.horizontal, KanjiJourneyTheme.spacingM)
            Spacer()
        }
