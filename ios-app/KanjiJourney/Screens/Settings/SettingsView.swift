...
struct SettingsView: View {
    ...
    private var sessionLengthPicker: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Session Length").font(KanjiJourneyTheme.bodyLarge)
            Text("Cards per session").font(KanjiJourneyTheme.bodySmall).foregroundColor(KanjiJourneyTheme.onSurfaceVariant)
            HStack(spacing: 4) {
                ForEach([10, 15, 20, 30], id: \.self) { length in
                    ...
                    Text("\(length)")
                        ...
                        .background(isSelected ? KanjiJourneyTheme.primary : Color.clear)
                        .cornerRadius(6)
    ...
    private var developerSection: some View {
        if viewModel.isDeveloper {
            settingsSection(title: "Developer", icon: "wrench.and.screwdriver") {
                clickableSetting("Dev Chat", "Chat with the KanjiJourney dev agent") { onDevChat() }
            }
        }
    }
    ...
    private var aboutSection: some View {
        settingsSection(title: "About", icon: "info.circle") {
            HStack {
                Text("Version").font(KanjiJourneyTheme.bodyLarge)
                Spacer()
                Text("1.0.0 (\(KanjiJourneyTheme.isPhone ? "iPhone" : "iPad"))").font(KanjiJourneyTheme.bodyMedium).foregroundColor(KanjiJourneyTheme.onSurfaceVariant)
            }
            .padding(.vertical, 4)
            ...
            Button(action: {
                ...
                Text("Sign Out").foregroundColor(KanjiJourneyTheme.error).fontWeight(.bold)
        ...
        VStack {
            ...
            Button(action: { showResetDialog = true }) {
                Text("Reset to Defaults")
                    .foregroundColor(KanjiJourneyTheme.error).fontWeight(.bold)
    ...
    private func settingsSection(title: String, icon: String, @ViewBuilder content: () -> some View) -> some View {
        VStack(alignment: .leading, spacing: 0) {
            HStack(spacing: 12) {
                Image(systemName: icon).foregroundColor(KanjiJourneyTheme.primary)
                Text(title).font(KanjiJourneyTheme.titleMedium).fontWeight(.bold)
            }
            .padding(.bottom, 12)
    ...
    private func switchSetting(_ title: String, _ subtitle: String, _ isOn: Bool, action: @escaping () -> Void) -> some View {
        HStack {
            VStack(alignment: .leading) {
                Text(title).font(KanjiJourneyTheme.bodyLarge)
                Text(subtitle).font(KanjiJourneyTheme.bodySmall).foregroundColor(KanjiJourneyTheme.onSurfaceVariant)
            }
            Spacer()
    ...
    private func clickableSetting(_ title: String, _ subtitle: String, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            VStack(alignment: .leading, spacing: 4) {
                Text(title).font(KanjiJourneyTheme.bodyLarge).foregroundColor(KanjiJourneyTheme.onSurface)
                Text(subtitle).font(KanjiJourneyTheme.bodyMedium).foregroundColor(KanjiJourneyTheme.primary).fontWeight(.bold)
            }
            .frame(maxWidth: .infinity, alignment: .leading)
            .padding(.vertical, 12)
        }
