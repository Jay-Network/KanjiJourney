...
actual class DatabaseDriverFactory {
    ...
    actual fun createDriver(): SqlDriver {
        NSLog("KanjiJourney [KN]: Opening DB at FULL PATH: %s", resolvedDbPath)
        ...
        NSLog("KanjiJourney [KN]: DB exists=%s, size=%d bytes", exists.toString(), size)
        ...
        val driver = NativeSqliteDriver(
            schema = KanjiJourneyDatabase.Schema,
            name = resolvedDbPath
        )

        // Verify kanji count right after opening
        driver.execute(null, "SELECT COUNT(*) FROM kanji", 0)
        NSLog("KanjiJourney [KN]: DB opened at %s (exists=%s, size=%d)", resolvedDbPath, exists.toString(), size)

        ensureNewTables(driver)
        return driver
    }

    fun createDatabase(driver: SqlDriver): KanjiJourneyDatabase {
    ...
    private fun ensureNewTables(driver: SqlDriver) {
        val statements = listOf(
            ...
            """CREATE TABLE IF NOT EXISTS coin_sync_queue (
                ...
                source_business TEXT NOT NULL DEFAULT 'kanjijourney',
    ...
    companion object {
        private const val DB_NAME = "kanjijourney.db"
    }
}
